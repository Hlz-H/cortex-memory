...
export function createTag(name: string, parentId?: string, description?: string): Tag {
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO tags (id, name, parent_id, description) VALUES (?, ?, ?, ?)').run(id, name, parentId || null, description || null);
...
export function deleteTag(id: string): boolean {
  ...
  if (children.cnt > 0) throw new Error('Cannot delete tag with child tags');
  const usage = db.prepare('SELECT COUNT(*) as cnt FROM memory_tags WHERE tag_id = ?').get(id) as { cnt: number };
  if (usage.cnt > 0) throw new Error(`Cannot delete tag: used by ${usage.cnt} memories`);
...
export function assignTagsToMemory(memoryId: string, tagIds: string[]): void {
  ...
  const tx = db.transaction(() => {
    ...
    for (const tagId of tagIds) stmt.run(memoryId, tagId);
  });
  tx();
}
